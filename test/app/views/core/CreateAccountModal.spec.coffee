...
#    jasmine.demoModal(modal)
  ...
  describe 'BasicInfoView', ->
    ...
    beforeEach ->
      ...
      spyOn basicInfoView, 'checkNameUniqueDebounced'
      
    it 'checks for name conflicts when the username input changes', ->
      basicInfoView.$('#username-input').val('test').trigger('input')
      expect(basicInfoView.checkNameUniqueDebounced).toHaveBeenCalled()
      
    describe 'checkNameUnique()', ->
      it 'shows an error if the name is taken', (done) ->
        request = jasmine.Ajax.requests.mostRecent()
        expect(request).toBeUndefined()
        basicInfoView.$('#username-input').val('test')
        basicInfoView.checkNameUnique()
        request = jasmine.Ajax.requests.mostRecent()
        expect(request).toBeDefined()
        request.respondWith({
          status: 200,
          responseText: JSON.stringify({
            conflicts: true
            givenName: 'test'
            suggestedName: 'test123'
          })
        })
        _.defer -> # wait for Promise
          expect(basicInfoView.$('#username-input').closest('.form-group').hasClass('has-error')).toBe(true)
          done()
